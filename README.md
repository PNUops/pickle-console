...
- **도메인 공개**: 서브도메인을 직접 정해 VM의 웹 서비스를 공개하고, 필요하면 다시
  내립니다.
- **도메인**: VM 없이 이름만 발급받아 A, AAAA, CNAME, TXT 레코드를 직접 편집합니다.
  승인을 기다리지 않고 그 자리에서 발급되며, 대상은 이 플랫폼 밖 서버여도 됩니다.
...
`/docs`에서 가입, 신청과 승인, 워크스페이스 공유, 가상머신, 도메인과 네트워크,
LLM API, 계정과 문제 해결을 로그인 없이 읽을 수 있습니다. 콘솔 메뉴의 사용 가이드는
현재 헤더와 선택한 워크스페이스 및 기관을 유지하며 왼쪽 리소스 메뉴를 문서 목차로
바꿉니다. 모바일에서는 상단 메뉴 버튼으로 목차를 엽니다. 문서 검색과 번호가 있는 절 목차,
공개 링크 복사, 이전·다음 문서와 원래 작업 화면으로 돌아가는 링크를 제공합니다.
...
일반 링크와 공유 링크의 경로 처리는 `src/lib/docs-paths.ts`가 맡습니다.
