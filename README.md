...
<table>
  <tr>
    <td><img src="assets/dashboard.png" width="420" alt="관리자 대시보드" /><br/><sub>관리자 대시보드. 승인 대기, VM 현황, 자원 요약</sub></td>
...
- **사용자 여정**: 회원가입부터 신청, 접속, 도메인 공개, 삭제까지 인프라를 몰라도 따라갈 수
  있게 안내합니다.
- **신청서**: OS와 사양을 각각 고르고 표시명과 희망 호스트명(슬러그), 희망 서브도메인을
  직접 정합니다.
...
- **관리자 콘솔**: 승인 큐와 판단을 돕는 자원 현황, 감사 로그, 드리프트 리포트, 관리자
  개입까지 다룹니다.
...
TypeScript 타입은 백엔드가 생성한 OpenAPI 스펙에서 만들어 저장소에 커밋합니다. 계약이
