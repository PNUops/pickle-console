...
import type { RequestKindAdmin, RequestKindModule } from './types'
...
/**
 * 이 콘솔 빌드가 모르는 종류의 신청을 위한 관리자 화면 폴백.
 *
 * api와 콘솔은 각자 일정으로 배포된다: 서버가 먼저 새 종류를 받기 시작하면,
 * 옛 번들을 돌리는 콘솔의 승인 큐에 그 신청이 들어온다. 설명할 수 없는 행이
 * 화면 전체를 무너뜨리는 대신 공통 필드만 보여주고, 결정은 막는다.
 */
const unknownKindAdmin: RequestKindAdmin = {
...
export function requestKindAdmin(type: string): RequestKindAdmin {
  return REQUEST_KIND_ADMINS[type] ?? unknownKindAdmin
}
...
export function useDecisionCatalogPrefetch() {
  useQueries({
    queries: Object.values(REQUEST_KIND_ADMINS).flatMap((kind) =>
