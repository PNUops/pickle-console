...
describe('워크스페이스 선택기', () => {
  test('워크스페이스를 고르면 보고 있던 화면이 그 워크스페이스로 바뀐다', async () => {
    ...
    await user.selectOptions(screen.getByRole('combobox', { name: '워크스페이스 선택' }), uuid(15))

    // 보고 있던 화면 그대로, 주소만 그 워크스페이스로 옮겨간다.
    await waitFor(() => expect(currentPath()).toBe(`/console/${uuid(15)}/resources`))
    expect(screen.getByRole('link', { name: 'VM' })).toHaveAttribute('href', `/console/${uuid(15)}/vms`)
