...
describe('알림 종(bell)', () => {
  test('사용자 상단 바에 읽지 않은 알림 수를 배지로 보여준다', async () => {
    ...
    const bell = await screen.findByRole('link', { name: '읽지 않은 알림 2개' })
    expect(bell).toHaveAttribute('href', '/console/notifications')
  ...
  test('관리자 상단 바 종은 관리자 알림함으로 연결된다', async () => {
    ...
    const bell = await screen.findByRole('link', { name: '읽지 않은 알림 1개' })
  ...
  test('읽지 않은 알림이 없으면 숫자 배지를 감춘다', async () => {
    ...
    const bell = await screen.findByRole('link', { name: '읽지 않은 알림 0개' })
...
describe('알림함', () => {
  test('알림을 누르면 읽음 처리되어 종 배지 수가 줄어든다', async () => {
    ...
    await screen.findByRole('link', { name: '읽지 않은 알림 2개' })

    // linkPath가 없는 알림 — 화면 이동 없이 읽음 처리만 된다.
    await user.click(await screen.findByText('VM 만료 7일 전'))
    await screen.findByRole('link', { name: '읽지 않은 알림 1개' })
  ...
  test('모두 읽음을 누르면 종 배지가 사라진다', async () => {
    ...
    await screen.findByRole('link', { name: '읽지 않은 알림 0개' })
  })
