...
describe('드리프트', () => {
  test('미해결 탭이 기본이고 유형 배지·대상·요약을 보여준다', async () => {
    ...
    const row = (await screen.findByText('VM #59 · vmid 5059 · pve1')).closest('tr')!
    expect(within(row).getByText('Proxmox에 없음')).toBeInTheDocument()
    expect(within(row).getByText(/broken-vm/)).toBeInTheDocument()
    // 해결된 발견은 미해결 탭에 없다.
    expect(screen.queryByText('vmid 9000 · pve1')).not.toBeInTheDocument()
  })
  ...
  test('해결 처리하면 미해결 목록에서 사라지고, 해결됨 탭은 해결자·자동 해소를 보여준다', async () => {
    ...
    await screen.findByRole('heading', { name: '드리프트' })
    const row = (await screen.findByText('vmid 9001 · pve1')).closest('tr')!
    ...
    await waitFor(() =>
      expect(screen.queryByText('vmid 9001 · pve1')).not.toBeInTheDocument(),
    )

    // 해결됨 탭: 수동 해결자는 이메일, 자동 해소는 라벨.
    await user.click(screen.getByRole('tab', { name: '해결됨' }))
    const manualRow = (await screen.findByText('vmid 9001 · pve1')).closest('tr')!
    expect(within(manualRow).getByText('sysadmin.lee@pusan.ac.kr')).toBeInTheDocument()
    const autoRow = screen.getByText('vmid 9000 · pve1').closest('tr')!
