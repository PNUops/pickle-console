...
describe('관리자 그룹 관리', () => {
  test('SYS_ADMIN은 전체 그룹을 종류·구성원 수와 함께 나열한다', async () => {
    ...
    expect(within(row).getByText('capstone-team3')).toBeInTheDocument()
  ...
  test('상세 드로어는 비활성·탈퇴 구성원까지 상태와 함께 보여준다', async () => {
    ...
    const drawer = within(await screen.findByRole('dialog', { name: '그룹 상세' }))
    ...
    expect(drawer.getByRole('link', { name: 'VM 보기' })).toHaveAttribute(
      'href',
      '/admin/vms?groupId=12',
    )
  })

  test('ORG_ADMIN은 자기 기관 그룹만 보고 기관 필터가 없다', async () => {
